...
const ReactivationRequests = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const res = await axiosSecure.get("/super-admin/reactivation-requests", { params });
      if (res.data.success) {
        setRequests(res.data.data);
      }
    } catch (error) {
      showError(error.response?.data?.message || "Failed to fetch reactivation requests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await axiosSecure.post(
        `/super-admin/reactivation-requests/${selectedRequest._id}/approve`,
        {
  ...
  const handleReject = async () => {
    ...
    try {
      const res = await axiosSecure.post(
        `/super-admin/reactivation-requests/${selectedRequest._id}/reject`,
        {
          reason: rejectReason,
        }
      );
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-primary/90 to-secondary p-8 shadow-lg">
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Reactivation Requests</h1>
            <p className="text-white/70 text-sm">Review and manage organization reactivation requests</p>
          </div>
          {requests.length > 0 && (
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message="There are no reactivation requests at the moment."
        />
      ) : (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-base-content/60 mb-3">
                    <div>
                  ...
                  {request.suspensionReason && (
                    <div className="mb-3">
                      <div className="flex items-center gap-2 text-error mb-1">
                        ...
                        <span className="font-medium text-sm">Suspension Reason:</span>
                      </div>
                      <p className="text-sm text-base-content/60 bg-error/5 rounded-lg p-3">
                        {request.suspensionReason}
                      </p>
                    </div>
                  )}
                  ...
                  {request.message && (
                    <div className="mb-3">
                      <span className="font-medium text-sm text-base-content/70">Request Message:</span>
                      <p className="text-sm text-base-content/60 bg-base-200/50 rounded-lg p-3 mt-1">
                        {request.message}
                      </p>
                    </div>
                  )}
                  ...
                  {request.rejectionReason && (
                    <div className="mb-3">
                      <span className="font-medium text-sm text-error">Rejection Reason:</span>
                      <p className="text-sm text-base-content/60 bg-error/5 rounded-lg p-3 mt-1">
                        {request.rejectionReason}
                      </p>
                    </div>
                  )}
                  ...
                  {request.adminComment && (
                    <div className="mb-3">
                      <span className="font-medium text-sm text-success">Admin Comment:</span>
                      <p className="text-sm text-base-content/60 bg-success/5 rounded-lg p-3 mt-1">
                        {request.adminComment}
                      </p>
                    </div>
                  )}
        ...
        <dialog className="modal modal-open">
          <div className="modal-box">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-lg text-base-content">Approve Reactivation Request</h3>
            ...
            <div className="space-y-4">
              <div className="bg-info/5 border border-info/20 rounded-lg p-3 text-sm text-base-content/70">
                This will reactivate the organization and restore full access.
              </div>
        ...
        <dialog className="modal modal-open">
          <div className="modal-box">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-lg text-base-content">Reject Reactivation Request</h3>
                <p className="text-xs text-base-content/50">
                  {selectedRequest.organizationName}
                </p>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-error/5 border border-error/20 rounded-lg p-3 text-sm text-base-content/70">
                The organization will remain suspended and the requester will be notified.
              </div>
              <div className="form-control">
                <label className="label">
